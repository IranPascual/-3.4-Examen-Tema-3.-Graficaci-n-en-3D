function creaSuelo(){
    var texture=new THREE.TextureLoader().load("Img/espacio.jpg");
...
function cargaMixamo(){
	...
	loader.load( 'Mixamo/walkbaby.fbx', function ( object ) {
...
function ponLuz(){
    light = new THREE.HemisphereLight( 0x6b075f, 0x8e9642 );
	light.position.set( posx, posy + 200, posz );
	scene.add( light );

	light = new THREE.DirectionalLight( 0x3093fc );
...
function ponEsferas(){
	for (var i = 0; i < numeroCosas; i++) {
		...
		var material = new THREE.MeshBasicMaterial({ color: 0x572c70 });
...
function colisionEsferas(){
	for (var i = 0; i < numeroCosas; i++) {
		if(Math.sqrt(Math.pow((posx - esferas[i].position.x), 2) + Math.pow((posz - esferas[i].position.z), 2)) < 100){
			 esferas[i].material.color.set( 0x32aaf0 );
		}
	}
}
